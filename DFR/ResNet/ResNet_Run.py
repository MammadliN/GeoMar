# -*- coding: utf-8 -*-
"""
Created on Sun Sep  8 17:31:29 2024
...
from ResNet.ResNet import ResNet_CAE, resnet_feature, train_resnet
...
def run_resnet(BS, num_epochs, latent_dim, patch, transform_minmax, transform_zscore, train_csv_path, val_csv_path, healthy_test_csv, cancer_test_csv):
    """
    Run the experiment by training and evaluating models based on ResNet features.
    ...
    image_output_folder = f"Result_images/ResNet/{patch}"
    ...
    # Initialize the backbone (ResNet feature extractor)
    backbone = resnet_feature()

    # Initialize the model
    model = ResNet_CAE(in_channels=1536, latent_dim=latent_dim).cuda()
    backbone.cuda()
    ...
    resnet_mse_min = train_resnet(
        model, backbone, criterion_mse, optimizer, BS, train_dataset_min, val_dataset_min,
        num_epochs=num_epochs, output_csv=f'CSVs/Loss/ResNet/{patch}/mse/MIN_{num_epochs}.csv',
        output_model=f"Models/ResNet/{patch}/mse/MIN_{num_epochs}.pth"
    )
    process_images_from_csv(backbone, resnet_mse_min, transform_minmax, cancer_test_csv, image_output_folder, num_epochs, loss="mse", norm="min", mask=masking)
    
    evaluate_model(
        backbone, resnet_mse_min, train_loader_min, healthy_test_csv, cancer_test_csv,
        transform_minmax, output_csv=f"CSVs/ROC/ResNet/{patch}/mse/MIN_{num_epochs}.csv",
        num_epochs=num_epochs, loss="mse", norm="min", output_folder=image_output_folder
    )

    resnet_euc1_min = train_resnet(
        model, backbone, criterion_euc1, optimizer, BS, train_dataset_min, val_dataset_min,
        num_epochs=num_epochs, output_csv=f'CSVs/Loss/ResNet/{patch}/euc1/MIN_{num_epochs}.csv',
        output_model=f"Models/ResNet/{patch}/euc1/MIN_{num_epochs}.pth"
    )
    process_images_from_csv(backbone, resnet_euc1_min, transform_minmax, cancer_test_csv, image_output_folder, num_epochs, loss="euc1", norm="min", mask=masking)
    
    evaluate_model(
        backbone, resnet_euc1_min, train_loader_min, healthy_test_csv, cancer_test_csv,
        transform_minmax, output_csv=f"CSVs/ROC/ResNet/{patch}/euc1/MIN_{num_epochs}.csv",
        num_epochs=num_epochs, loss="euc1", norm="min", output_folder=image_output_folder
    )
    
    resnet_euc2_min = train_resnet(
        model, backbone, criterion_euc2, optimizer, BS, train_dataset_min, val_dataset_min,
        num_epochs=num_epochs, output_csv=f'CSVs/Loss/ResNet/{patch}/euc2/MIN_{num_epochs}.csv',
        output_model=f"Models/ResNet/{patch}/euc2/MIN_{num_epochs}.pth"
    )
    process_images_from_csv(backbone, resnet_euc2_min, transform_minmax, cancer_test_csv, image_output_folder, num_epochs, loss="euc2", norm="min", mask=masking)
    
    evaluate_model(
        backbone, resnet_euc2_min, train_loader_min, healthy_test_csv, cancer_test_csv,
        transform_minmax, output_csv=f"CSVs/ROC/ResNet/{patch}/euc2/MIN_{num_epochs}.csv",
        num_epochs=num_epochs, loss="euc2", norm="min", output_folder=image_output_folder
    )

    # resnet_mse_zs = train_resnet(
    #     model, backbone, criterion_mse, optimizer, BS, train_dataset_zs, val_dataset_zs,
    #     num_epochs=num_epochs, output_csv=f'CSVs/Loss/ResNet/{patch}/mse/ZS_{num_epochs}.csv',
    #     output_model=f"Models/ResNet/{patch}/mse/ZS_{num_epochs}.pth"
    # )
    # process_images_from_csv(backbone, resnet_mse_zs, transform_zscore, cancer_test_csv, image_output_folder, num_epochs, loss="mse", norm="zs", mask=masking)

    # evaluate_model(
    #     backbone, resnet_mse_zs, train_loader_zs, healthy_test_csv, cancer_test_csv,
    #     transform_zscore, output_csv=f"CSVs/ROC/ResNet/{patch}/mse/ZS_{num_epochs}.csv",
    #     num_epochs=num_epochs, loss="mse", norm="zs", output_folder=image_output_folder
    # )

    # resnet_euc1_zs = train_resnet(
    #     model, backbone, criterion_euc1, optimizer, BS, train_dataset_zs, val_dataset_zs,
    #     num_epochs=num_epochs, output_csv=f'CSVs/Loss/ResNet/{patch}/euc1/ZS_{num_epochs}.csv',
    #     output_model=f"Models/ResNet/{patch}/euc1/ZS_{num_epochs}.pth"
    # )
    # process_images_from_csv(backbone, resnet_euc1_zs, transform_zscore, cancer_test_csv, image_output_folder, num_epochs, loss="euc1", norm="zs", mask=masking)    
  
    # evaluate_model(
    #     backbone, resnet_euc1_zs, train_loader_zs, healthy_test_csv, cancer_test_csv,
    #     transform_zscore, output_csv=f"CSVs/ROC/ResNet/{patch}/euc1/ZS_{num_epochs}.csv",
    #     num_epochs=num_epochs, loss="euc1", norm="zs", output_folder=image_output_folder
    # )
    
    # resnet_euc2_zs = train_resnet(
    #     model, backbone, criterion_euc2, optimizer, BS, train_dataset_zs, val_dataset_zs,
    #     num_epochs=num_epochs, output_csv=f'CSVs/Loss/ResNet/{patch}/euc2/ZS_{num_epochs}.csv',
    #     output_model=f"Models/ResNet/{patch}/euc2/ZS_{num_epochs}.pth"
    # )
    # process_images_from_csv(backbone, resnet_euc2_zs, transform_zscore, cancer_test_csv, image_output_folder, num_epochs, loss="euc2", norm="zs", mask=masking)    
  
    # evaluate_model(
    #     backbone, resnet_euc2_zs, train_loader_zs, healthy_test_csv, cancer_test_csv,
    #     transform_zscore, output_csv=f"CSVs/ROC/ResNet/{patch}/euc2/ZS_{num_epochs}.csv",
